...
public class TreasureHuntCommand implements CommandExecutor {
		...
		if (sender instanceof Player) {
			if (!(type.isEmpty() || type.equals(""))) {
				if (!plugin.getTTypeManager().exists(type)) {
					...
					sender.sendMessage(ChatColor.DARK_RED + "That Treasure type does not exist");
			} else
				sender.sendMessage(ChatColor.YELLOW + command + "createtype [type]");
		} else
			sender.sendMessage(ChatColor.DARK_RED + "Only players can use that command");
	}
			...
			if (!(name.isEmpty() || name.equals("") || type.isEmpty() || type.equals(""))) {
				if (!plugin.getTCManager().exists(name)) {
					if (plugin.getTTypeManager().exists(type)) {
						...
						sender.sendMessage(ChatColor.DARK_RED + "Invalid Type: use " + command
								+ "createtype [type] to create a type first");
				} else
					sender.sendMessage(ChatColor.DARK_RED + "The treasure chest with that name already exists");
			} else
		...
		if (sender instanceof Player) {
			if (name.isEmpty() || name.equals("")) {
				...
				if (block != null) {
					if (block.getType().equals(Material.CHEST)) {
						...
						if (plugin.getTCManager().exists(loc)) {
							String success = plugin.getTCManager().removeTreasureChest(name) ? "§2Successfully removed"
									: "§4Failed to remove";
							sender.sendMessage(success + " the treasure chest you are looking ");
						} else
							sender.sendMessage(
									ChatColor.DARK_RED + "The chest you are looking at is not a treasure chest");
					} else
						sender.sendMessage(ChatColor.DARK_RED + "The block you are looking at is not a chest");
				} else
					sender.sendMessage(ChatColor.DARK_RED + "The block you are looking at is not a chest");
				...
				if (plugin.getTCManager().exists(name)) {
					...
					sender.sendMessage(ChatColor.DARK_RED + "The treasure chest with that name does not exist");
			}
		} else {
			if (!(name.isEmpty() || name.equals(""))) {
				if (plugin.getTCManager().exists(name)) {
					...
					if (plugin.getTCManager().exists(loc)) {
						if (plugin.getTTypeManager().exists(type)) {
							...
							sender.sendMessage(ChatColor.DARK_GREEN
									+ "Successfully changed type of treasure chest you were looking at to " + type);
						} else
							sender.sendMessage(ChatColor.DARK_RED + "Invalid Type: use " + command
									+ "createtype [type] to create a type first");
					} else
						sender.sendMessage(ChatColor.DARK_RED + "The chest you are looking at is not a treasure chest");
				} else
					sender.sendMessage(ChatColor.DARK_RED + "The block you are looking at is not a chest");
			} else {
				if (plugin.getTCManager().exists(name)) {
			...
			if (!(name.isEmpty() || name.equals(""))) {
				if (plugin.getTTypeManager().exists(type)) {
					if (plugin.getTCManager().exists(name)) {
						plugin.getTCManager().changeType(name, type);
						sender.sendMessage(
								ChatColor.DARK_GREEN + "Successfully changed type of " + name + " to " + type);
					} else
						sender.sendMessage(ChatColor.DARK_RED + "The treasure chest with that name does not exist");
				} else
					sender.sendMessage(ChatColor.DARK_RED + "Invalid Type: use " + command
			...
			if (name.isEmpty() || name.equals("")) {
				...
				if (block.getType().equals(Material.CHEST)) {
					...
					if (plugin.getTCManager().exists(loc)) {
						...
						sender.sendMessage(
								ChatColor.DARK_GREEN + "The treasure chest with name " + name + " has been reset");
					} else
						sender.sendMessage(ChatColor.DARK_RED + "The chest you are looking at is not a treasure chest");
				} else
					sender.sendMessage(ChatColor.DARK_RED + "The block you are looking at is not a chest");
	...
	private void pluginInfo(CommandSender sender) {
		...
		sender.sendMessage(
				ChatColor.YELLOW + "Version" + ChatColor.GRAY + ": " + ChatColor.DARK_GREEN + p.getVersion());
		sender.sendMessage(
				ChatColor.YELLOW + "Made by" + ChatColor.GRAY + ": " + ChatColor.DARK_GREEN + p.getAuthors());
		sender.sendMessage(
				ChatColor.YELLOW + "Description" + ChatColor.GRAY + ": " + ChatColor.DARK_GREEN + p.getDescription());
	}
